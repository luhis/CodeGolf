    ...
    public class ChallengeSetShould
    {
        ...
        public void ReturnTrueResultWhenCorrect()
        {
            var a = (IChallengeSet) new ChallengeSet<string>("a", "b", new[] {typeof(string)},
                new[] {new Challenge<string>(new object[] {"test"}, "test")});
            var r = a.GetResults(new CompileResult(o => Task.FromResult(Option.Some<OneOf.OneOf<object, object[]>, string>("test")))).Result;
            r.Single().Error
        ...
        public void ReturnFalseResultWhenIncorrect()
        {
            var a = (IChallengeSet)new ChallengeSet<string>("a", "b", new[] { typeof(string) },
                new[] { new Challenge<string>(new object[] { "test" }, "test") });
            var r = a.GetResults(new CompileResult(o => Task.FromResult(Option.Some< OneOf.OneOf<object, object[]>, string>("testXX")))).Result;
            r.Single().Error
